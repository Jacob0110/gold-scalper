...
// Styles (請確保你有建立 App.css 並貼上我上一步給的內容)
import "./App.css";
...
export default function App() {
    ...
    try {
      ...
      if (playPromise !== undefined) {
        playPromise.catch((error) => {
          // 這裡是用來捕捉 "Auto-play policy" 錯誤的
          console.warn(
            "Audio play blocked by browser (User didn't interact yet):",
            error
          );
        });
      }
  ...
  const sendSystemNotification = (title, body) => {
    ...
    if (Notification.permission === "granted") {
      new Notification(title, { body, icon: "/vite.svg" }); // icon 可換成你的 logo
        ...
        ws.onmessage = (e) => {
          ...
          if (!isAboveEma) {
            ...
            if (candle.isFinal && !activeSignalRef.current) {
              ...
              // 2. 核心邏輯：先鎖定狀態 (這是最重要的，確保交易進行)
              setActiveSignal(sig);
              activeSignalRef.current = sig;

              // 3. 附加邏輯：通知 (必須放在 Try-Catch 裡面，壞了也不影響交易)
              try {
                console.log("Triggering notification..."); // Debug log

                playAlertSound(); // 嘗試播放聲音

                sendSystemNotification(
                  "JINGUO SIGNAL 🚀",
                  `Buy @ ${entry} | TP: ${target} | SL: ${stop}`
                );
              } catch (error) {
                // 就算 iPhone 擋住聲音，這裡只會印 error，不會弄死你的交易機器人
                console.warn("Notification error (Trade is safe):", error);
              }
            }
          } else {
            tip = "👀 等待綠K (EMA20之上)...";
      ...
      <div className="app-header">
        ...
        <button
          onClick={() => {
            ...
            playAlertSound(); // 測試聲音，同時解鎖瀏覽器自動播放限制
